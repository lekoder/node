# Maintaining V8 in Jayo.js

## Background

V8 follows the Chromium release schedule. The support horizon for Chromium is
very different from the support horizon that Jayo.js needs to provide to its
users. As a result Jayo.js needs to support a version of V8 for quite a bit
...
Jayo.js are maintained.

This document attempts to document the current processes and proposes a workflow
for maintaining the V8 branches in Jayo.js LTS and Current releases and how the
Jayo.js and V8 teams at Google can help.
...
## Jayo.js Support Requirements

At any given time Jayo.js needs to be maintaining a few different V8 branches
...
The versions of V8 used in Jayo.js v4.x and v6.x have already been abandoned by
upstream V8. However, Jayo.js needs to continue supporting these branches for
many months (Current branches) or several years (LTS branches).

## Maintenance Process

Once a bug in Jayo.js has been identified to be caused by V8, the first step is
to identify the versions of Jayo.js and V8 affected. The bug may be present in
...
* Backports identified by the V8 team. Bugs identified by upstream V8 that we haven't encountered in Jayo.js yet.
...
* Start by opening a bug upstream [using this template](https://bugs.chromium.org/p/v8/issues/entry?template=Jayo.js%20upstream%20bug).
* Make sure to include a link to the corresponding Jayo.js issue (if one exists).
...
* Once the bug is fixed it may still need backporting, if it exists in other V8 branches that are still active or are branches that Jayo.js cares about. Follow the process for backporting below.
...
* A tracking bug is needed to request a backport.
    * If there isn't already a V8 bug tracking the fix, open a new merge request bug using this [Jayo.js specific template](https://bugs.chromium.org/p/v8/issues/entry?template=Jayo.js%20merge%20request).
...
* It is possible that the merge request may not get approved, for example if it is considered to be a feature or otherwise too risky for V8 stable. In such cases we float the patch on the Jayo.js side. See the process on 'Backporting to Abandoned branches'.
* Once the fix has been merged upstream, it can be picked up during an update of the V8 branch, (see below).

### Backporting to Abandoned Branches

Abandoned V8 branches are supported in the Jayo.js V8 repository. The fix needs
to be cherry-picked in the Jayo.js repository and V8-CI must test the change.
...
* For each abandoned V8 branch corresponding to an LTS branch that is affected by the bug:
    ...
    * Increase the patch level version in v8-version.h. This will not cause any problems with versioning because V8 will not publish other patches for this branch, so Jayo.js can effectively bump the patch version.
    ...
    * Run the Jayo.js [V8-CI](https://ci.nodejs.org/job/node-test-commit-v8-linux/) in addition to the [Jayo.js CI](https://ci.nodejs.org/job/node-test-pull-request/).
...
5.2 and 5.3, and not into V8 5.1 (since it was already abandoned). Since Jayo.js
...
* Modify the commit message to match the format we use for V8 backports and replace yourself as the author. `git commit --amend --reset-author`. You may want to add extra description if necessary to indicate the impact of the fix on Jayo.js. In this case the original issue was descriptive enough. Example:
```console
...
PR-URL: <pr link>
```
* Open a PR against the `v6.x-staging` branch in the Jayo.js repo. Launch the normal and [V8-CI](https://ci.nodejs.org/job/node-test-commit-v8-linux/) using the Jayo.js CI system. We only needed to backport to `v6.x` as the other LTS branches weren't affected by this bug.

### Backports Identified by the V8 team

For bugs found through the browser or other channels, the V8 team marks bugs
that might be applicable to the abandoned branches in use by Jayo.js. This is
...
*   `NodeJS-Backport-Review` ([V8](https://bugs.chromium.org/p/v8/issues/list?can=1&q=label%3ANodeJS-Backport-Review), [Chromium](https://bugs.chromium.org/p/chromium/issues/list?can=1&q=label%3ANodeJS-Backport-Review)): to be reviewed if this is applicable to abandoned branches in use by Jayo.js. This list if regularly reviewed by the Jayo.js team at Google to determine applicability to Jayo.js.
*   `NodeJS-Backport-Approved` ([V8](https://bugs.chromium.org/p/v8/issues/list?can=1&q=label%3ANodeJS-Backport-Approved), [Chromium](https://bugs.chromium.org/p/chromium/issues/list?can=1&q=label%3ANodeJS-Backport-Approved)): marks bugs that are deemed relevant to Jayo.js and should be backported.
*   `NodeJS-Backport-Done` ([V8](https://bugs.chromium.org/p/v8/issues/list?can=1&q=label%3ANodeJS-Backport-Done), [Chromium](https://bugs.chromium.org/p/chromium/issues/list?can=1&q=label%3ANodeJS-Backport-Done)): Backport for Jayo.js has been performed already.
*   `NodeJS-Backport-Rejected` ([V8](https://bugs.chromium.org/p/v8/issues/list?can=1&q=label%3ANodeJS-Backport-Rejected), [Chromium](https://bugs.chromium.org/p/chromium/issues/list?can=1&q=label%3ANodeJS-Backport-Rejected)): Backport for Jayo.js is not desired.
...
Jayo.js keeps a vendored copy of V8 inside of deps/ directory. In addition
Jayo.js may need to float patches that do not exist upstream. This means that
some care may need to be taken to update the vendored copy of V8.

### Minor updates (patch level)

Because there may be floating patches on the version of V8 in Jayo.js, it is
safest to apply the patch level updates as a patch. For example, imagine that
upstream V8 is at 5.0.71.47 and Jayo.js is at 5.0.71.32. It would be best to
compute the diff between these tags on the V8 repository, and then apply that
patch on the copy of V8 in Jayo.js. This should preserve the patches/backports
that Jayo.js may be floating (or else cause a merge conflict).

The rough outline of the process is:

```shell
# Assuming your fork of Jayo.js is checked out in $NODE_DIR
# and you want to update the Jayo.js master branch.
...
We upgrade the version of V8 in Jayo.js master whenever a V8 release goes stable
...
1. Replace the copy of V8 in Jayo.js with a fresh checkout of the latest stable V8 branch. Special care must be taken to recursively update the DEPS that V8 has a compile time dependency on (at the moment of this writing, these are only trace_event and gtest_prod.h)
...
To replace the copy of V8 in Jayo.js, use the '[update-v8](https://gist.github.com/targos/8da405e96e98fdff01a395bed365b816)' script<sup>2</sup>. For example, if you want to replace the copy of V8 in Jayo.js with the branch-head for V8 5.1 branch:
...
The fact that Jayo.js keeps a vendored, potentially edited copy of V8 in deps/
...
* The process to update the version of V8 in Jayo.js could be automated to track the tips of various V8 branches in nodejs/v8.
...
* It would make it simpler to setup an automated build that tracks Jayo.js master + V8 lkgr integration build.

This would require some tooling to:

* A script that would update the V8 in a specific Jayo.js branch with V8 from upstream (dependent on branch abandoned vs. active).
...
only affects Jayo.js and not the browser. At the moment we don't have a
...
would also help with auditing the floating patches in the Jayo.js commit
history.
...
<sup>1</sup>Jayo.js 0.12 and older are intentionally omitted from this document as their support is ending soon.
